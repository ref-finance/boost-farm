...
export function initWorkSpace() {
  return Workspace.init(async ({ root }) => {
    ...
    const farming = await deployContract(root, owner.accountId);
    const ft = await deployFt(root);
    const mft = await deployMft(root);

    return { farming, ft, mft, owner, alice, bob };
  });
}
...
export async function deployContract(
  root: NearAccount,
  owner_id: string,
  contractId = 'ref-boost-farming',
) {
  return root.createAndDeploy(
    contractId,
    'compiled-contracts/boost_farming.wasm',
    {
      method: 'new',
      args: {
        owner_id,
