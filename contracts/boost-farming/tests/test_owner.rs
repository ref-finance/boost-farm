...
fn test_set_owner(){
    ...
    assert_err!(
        e.grant_next_owner(&users.alice, &e.owner, 0),
        "Requires attached deposit of exactly 1 yoctoNEAR"
    );

    // 2: E002_NOT_ALLOWED
    assert_err!(
        e.grant_next_owner(&users.alice, &e.owner, 1),
        E002_NOT_ALLOWED
    );

    // success
    e.grant_next_owner(&e.owner, &users.alice, 1).assert_success();
    e.accept_next_owner(&users.alice, 1).assert_success();
    e.confirm_next_owner(&e.owner, 1).assert_success();
...
fn test_operators(){
    ...
    e.remove_operators(&e.owner, vec![&users.bob], 1).assert_success();
    assert_eq!(e.get_metadata().operators, vec![]);
}
